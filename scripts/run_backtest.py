...
async def _fetch_all_data(pair: str, start: str, end: str, start_ms: int, end_ms: int, interval: str):
    ...
    return candles, fng, funding, btc_dom, fed_rate, dxy, fut_vol
...
def _build_snapshot(candles, i, ts, c, v, window, fng, funding, btc_dom, fed_rate, dxy, fut_vol):
    ...
    return DataSnapshot(
        ...
        onchain=OnchainData(
            open_interest=fut_volume,
            liquidations_24h={"volume_ratio": vol_ratio, "futures_volume": fut_volume},
        ),
        news=NewsSentiment(
            sentiment_score=sentiment, key_events=events, headlines=[f"BTC at ${c:,.0f}, Fear&Greed={fng_val}"]
        ),
        macro=MacroData(fear_greed_index=fng_val, btc_dominance=dom_val, fed_rate=fed_val, dxy=dxy_val),
    ), fng_val


async def _get_action(graph, snapshot, candles, i, window, equity, c, t0):
...
async def main():
    ...
    candles, fng, funding, btc_dom, fed_rate, dxy, fut_vol = await _fetch_all_data(
    ...
    for i in range(LOOKBACK, len(candles)):
        ...
        t0 = time.time()
        snapshot, fng_val = _build_snapshot(candles, i, ts, c, v, window, fng, funding, btc_dom, fed_rate, dxy, fut_vol)

        action, confidence, size_pct, elapsed, adx = await _get_action(
            graph, snapshot, candles, i, window, equity, c, t0
        )
        ...
        if _should_execute_trade(action, position, stopped, step, paused_until, days_since_flip):
            position, entry_price, equity, new_trades = _execute_trade(
                action, position, equity, c, entry_price, size_pct, date
            )
    ...
    print(f"\n{'=' * 50}", flush=True)
    print("LLM Strategy (deepseek-chat, lite graph)", flush=True)
