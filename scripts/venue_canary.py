...
async def _open_target(connection_id: str):
    ...
    return snapshot, connection, await registry.require(connection.adapter_id).connect(connection, credentials)
...
async def run_simulated_canary(session, pair: Pair) -> dict[str, Any]:
    ...
    try:
        ...
        protection = await session.replace_protection(
            ProtectionSpec(pair, "long", amount, quote.last * Decimal("0.98"), quote.last * Decimal("1.02"))
        )
        result["protection_ids"] = tuple(protection.protection_ids)
        ...
        await session.place_order(OrderIntent(pair, "sell", amount, "market", None, True))
        await session.cancel_protection(tuple(protection.protection_ids))
...
async def _main(options: argparse.Namespace) -> dict[str, Any]:
    snapshot, connection, session = await _open_target(options.connection)
    pair = Pair.parse(options.pair)
    if options.live_read_only:
        try:
            await session.fetch_portfolio(pair)
            await session.list_open_state(pair)
            return {
                "status": "completed",
