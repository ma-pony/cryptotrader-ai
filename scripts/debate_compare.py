...
async def build_snapshot(candles, fng, funding, btc_dom, fed, dxy, target_date):
    ...
    return DataSnapshot(
        ...
        market=MarketData(
            ...
            funding_rate=funding.get(date_str, 0.0),
        ...
        macro=MacroData(
            fear_greed_index=fng.get(date_str, 50),
            btc_dominance=btc_dom.get(date_str, 0),
            fed_rate=fed.get(date_str, 0),
            dxy=dxy.get(date_str, 0),
        ),
    ), c
