...
- **spec 017b cases.jsonl schema 验证**（from #03）—— DefaultMemoryProvider `_format_case` 期望字段是否与现网 cases.jsonl 一致
- **spec 017b prompt_template 字段使用情况**（from #03）—— config/default.toml 中是否仍有 `prompt_template = "..."` 引用
- **spec 017b graph.py 是否含 agent 实例化**（from #03）—— FR-Y35 实施时确认
