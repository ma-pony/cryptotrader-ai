...
export const runtimeConfigFixture = (overrides: Record<string, unknown> = {}) => RuntimeConfigSchema.parse({
  ...
  document: {
    system: { active: true }, security: { enabled: false, access_credential_configured: false, access_credential_updated_at: null }, market_data: { news_credential_configured: false, news_credential_updated_at: null, source_id: 'binance', parameters: [] },
    ...
    risk: { position: { max_single_pct: 0.2, max_total_exposure_pct: 0.8, max_margin_used_pct: 0.5 }, loss: { max_drawdown_pct: 0.15 } },
    execution: { connections: [], books: [], live_order_execution_enabled: false }, hitl: { approval_ttl_minutes: 15 }, scheduler: { enabled: false, pairs: ['BTC/USDT'], interval_minutes: 15, daily_summary_hour: 8 }, triggers: { enabled: false, max_rules: 10, ws_reconnect_max_s: 30, funding_rate_poll_interval_minutes: 15 }, notifications: { webhook_url: '', enabled: false, webhook_timeout: 10, events: [] }, infrastructure: { redis_url: 'redis://localhost:6379/0' }, observability: { otlp_endpoint: '' },
  },
  ...overrides,
});
