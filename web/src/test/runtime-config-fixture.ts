...
export const runtimeConfigFixture = (overrides: Record<string, unknown> = {}) => RuntimeConfigSchema.parse({
  ...
  document: {
    system: { active: true }, market_data: { source_id: 'binance', parameters: [] },
    llm: { base_url: 'https://gateway.example', streaming_models: [], default_temperature: 0.2, timeout: 30, prompt_caching: false, retry: { max_attempts: 1, retry_base_delay_s: 1, retry_backoff_factor: 2, retry_jitter: false }, model_costs: [], models: { analysis: 'analysis', debate: 'debate', committee_summary: 'summary', tech_agent: 'tech', chain_agent: 'chain', news_agent: 'news', macro_agent: 'macro', fallback: 'fallback', timeout_seconds: 30 } },
