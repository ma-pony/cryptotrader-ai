  ...
  for (const book of books.filter((item) => item.enabled)) {
    ...
    for (const allocation of enabled) {
      ...
      if (!connection || !connection.enabled || !eligible(book.capital_scope, connection.environment))
        errors.push(
          book.capital_scope === 'simulated'
            ? '模拟资金池不能包含实盘连接或未启用连接'
            : '实盘资金池只能包含已启用的 live 连接',
        );
      if (local.has(allocation.connection_id)) errors.push('同一资金池不能重复 allocation');
      local.add(allocation.connection_id);
      if (used.has(allocation.connection_id)) errors.push('同一启用连接不能进入多个启用资金池');
  ...
  return (
    <article className="rounded-xl border border-border bg-muted/10 p-4">
      ...
      <div className="mt-4 space-y-2 border-t border-border pt-3">
        {allowed.map((connection) => {
          ...
          return (
            <div key={connection.id} className="flex items-center gap-3 text-sm">
              <input
                aria-label={`${connection.label} 启用`}
                type="checkbox"
              ...
              <input
                aria-label={`${connection.label} 权重`}
        ...
        {allowed.length === 0 ? <p className="text-sm text-muted-foreground">当前作用域没有可用连接。</p> : null}
      </div>
