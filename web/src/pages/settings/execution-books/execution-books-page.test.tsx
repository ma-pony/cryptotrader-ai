...
describe('ExecutionBooksPage', () => {
  it('blocks mixed capital scopes and previews exact weighted notionals', () => {
    ...
    expect(validateBooks([{ id: 'sim', label: '模拟资金池', capital_scope: 'simulated', enabled: true, hitl_required: true, allocations: [{ connection_id: 'live', enabled: true, weight: 1 }] }], [{ id: 'live', label: 'Live', adapter_id: 'okx', environment: 'live', enabled: true, leverage: 1, margin_mode: 'cross', parameters: {} }])).toContain('模拟资金池不能包含实盘连接');
  });
});
