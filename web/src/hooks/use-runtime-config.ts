...
export const decodeJsonValue = (value: JsonValueOut): RuntimeJsonValue => {
  switch (value.kind) {
    ...
    case 'array': return value.items.map(decodeJsonValue);
    case 'object': return Object.fromEntries(value.entries.map((entry) => [entry.key, decodeJsonValue(entry.value)]));
  }
};
...
export const toRuntimeDocument = (response: RuntimeConfig['document']): RuntimeDocument => ({
  ...
  signals: { ...response.signals, components: response.signals.components.map((component) => ({ ...component, parameters: decodeEntries(component.parameters) })) },
  execution: { ...response.execution, connections: response.execution.connections.map(({ credential_configured: _configured, credential_updated_at: _updatedAt, parameters, ...connection }) => ({ ...connection, parameters: decodeEntries(parameters) })) },
});

export const useRuntimeConfig = () => {
  const client = useQueryClient();
  const [conflict, setConflict] = useState(false);
  const query = useQuery({ queryKey: RUNTIME_CONFIG_QUERY_KEY, queryFn: () => apiClient.get('/api/config', RuntimeConfigSchema) });
  const mutation = useMutation({
    mutationFn: (document: RuntimeDocument) => {
      if (query.data?.revision === undefined) return Promise.reject(new Error('Runtime config revision is unavailable'));
      return apiClient.put('/api/config', { expected_revision: query.data.revision, document }, RuntimeConfigSchema);
    },
    onSuccess: (saved) => { setConflict(false); client.setQueryData(RUNTIME_CONFIG_QUERY_KEY, saved); },
  ...
  const credentialStates = useMemo(() => Object.fromEntries((query.data?.document.execution.connections ?? []).map((connection) => [connection.id, { configured: connection.credential_configured, updatedAt: connection.credential_updated_at }])), [query.data]);
  return { revision: query.data?.revision, document, credentialStates, setupRequired: query.data?.setup_required ?? false, updatedAt: query.data?.updated_at, replace: mutation.mutateAsync, reload: () => query.refetch(), conflict, isLoading: query.isLoading, isError: query.isError, isSaving: mutation.isPending };
};
