  ...
  proposal: {
    ...
    risk: {
      ...
      connection_targets: [
        {
          book_id: 'real-book',
          connection_id: 'okx-live',
          weight: '1.0',
          book_equity: '30000',
          target_exposure: '0.35',
          target_signed_notional: '10500',
        },
      ],
    ...
    connection_risks: [
      { connection_id: 'okx-live', passed: false, risk_increase: true, reason: 'leverage cap', operation: 'preflight' },
    ],
    connection_plans: [
      {
        book_id: 'real-book',
        connection_id: 'okx-live',
        pair: { symbol: 'ETH/USDT' },
        current_signed_notional: '1000',
        target_signed_notional: '10500',
        delta_signed_notional: '9500',
        current_signed_amount: '0.25',
        target_signed_amount: '2.625',
        delta_signed_amount: '2.375',
        post_fill_signed_amount: '2.625',
        quote: { pair: { symbol: 'ETH/USDT' }, bid: '3999', ask: '4001', last: '4000' },
        execution_price: '4001',
        amount: '2.375',
        side: 'buy',
        reduce_only: false,
        market_type: 'swap',
        stop_loss: '3800',
        take_profit: '4400',
        old_protection_ids: ['sl-old', 'tp-old'],
        capabilities: {
          market_types: ['swap'],
          ...
          account_reads: ['balances', 'positions', 'orders'],
          exit_operations: ['cancel_order', 'cancel_protection', 'close_position'],
...
describe('approval book plan', () => {
  ...
  it('keeps the frozen complete proposal visible through a partial attention outcome after approval', async () => {
    const fetchMock = vi.fn((url: string, init?: RequestInit) => {
      if (String(url) === '/api/hitl/pending' && init?.method === 'GET') {
        return Promise.resolve(
          json(
            fetchMock.mock.calls.filter(([called]) => String(called) === '/api/hitl/pending').length === 1
              ? [approval]
              : [],
          ),
        );
    ...
    for (const text of [
      ...
      '资金范围: real',
      '状态: 等待中',
      '创建时间: 2026-08-29T00:00:00Z',
      '请求敞口: 0.40',
      '目标敞口: 0.35',
      'okx-live 0.35',
      'okx-live leverage cap',
      'bybit-live',
      'bybit-live unavailable',
      'okx-live · ETH/USDT',
      '3999/4001/4000',
      '数量: 2.375',
      '名义价值: 10500',
    ...
    const confirm = await screen.findByRole('button', { name: '确认批准' });
    ...
    const post = fetchMock.mock.calls.find(
      ([url, init]) => String(url) === '/api/hitl/approval-1/respond' && (init as RequestInit).method === 'POST',
    );
    ...
    expect(screen.getByRole('link', { name: '查看周期' })).toHaveAttribute('href', '/decisions/cycle-partial');
  ...
  it.each([409, 500])('keeps the proposal and reports a localized error when respond returns %i', async (status) => {
    const fetchMock = vi.fn((url: string, init?: RequestInit) => {
      ...
      if (String(url) === '/api/hitl/approval-1/respond' && init?.method === 'POST')
        return Promise.resolve(apiError(status));
      throw new Error(`unexpected request ${String(url)} ${init?.method}`);
    });
    vi.stubGlobal('fetch', fetchMock);
    renderQueue();
