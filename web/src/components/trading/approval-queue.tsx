...
export const ApprovalQueue = ({ bookId }: { bookId?: string }) => {
    ...
    <div className="space-y-3">
      ...
      {outcome ? (
        <div className="rounded border border-amber-500 bg-amber-500/10 p-3 text-sm">
          ...
          <Link className="underline" to={`/decisions/${outcome.cycle_id}`}>
            {t('hitl.view_cycle')}
          </Link>
      ...
      {pending?.filter((approval) => !bookId || approval.book_id === bookId).map((approval) => (
        <ApprovalItem key={approval.approval_id} approval={approval} onOutcome={setOutcome} />
      ))}
      {!isLoading && !isError && !pending?.filter((approval) => !bookId || approval.book_id === bookId).length ? (
