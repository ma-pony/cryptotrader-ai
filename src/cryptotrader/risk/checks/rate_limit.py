...
class RateLimitCheck:
    ...
    async def evaluate(self, request: RiskRequest, portfolio: dict) -> RiskCheckResult:
        ...
        if hourly >= self._max_hour:
            return RiskCheckResult(passed=False, reason=f"Hourly trade limit ({self._max_hour}) reached")
        if daily >= self._max_day:
            return RiskCheckResult(passed=False, reason=f"Daily trade limit ({self._max_day}) reached")
