...
主分支部署由 GitHub Actions 构建 API/Web 镜像，并以提交 SHA 发布到 GHCR。VPS 只拉取该不可变镜像、执行 schema 迁移并以 `--no-build` 启动服务，不在生产机安装依赖或编译镜像。若第一次切换镜像时磁盘不足，工作流只移除旧的 API/Web 容器和镜像后重试，所有命名数据卷都会保留。
