...
默认行情源的退出计算周期（`market_data.parameters.timeframe`）和 K 线数量（`market_data.parameters.limit`）专门用于统一 ATR 止盈止损。实时分析与回测读取同一份退出需求，明确使用该周期计算 ATR；组件启用顺序不会改变退出周期。

组件自身的 K 线周期决定其分析输入；`market_data.timeframe` 用于评估参考 K 线；信号评估窗口决定何时核对预测；调度或回测步长决定何时运行。这些含义相互独立，修改评估周期不会替换退出策略的 ATR 输入。

例如组件使用 4 小时 K 线、退出周期设置为 1 小时，止盈止损仍使用 1 小时 ATR。即使所有组件都不请求退出周期的 K 线，实时分析与回测也会单独补齐所需数据。字段保持不变，无需为此迁移运行配置。
