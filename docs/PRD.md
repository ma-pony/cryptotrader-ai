...
| 变体 | 函数 | 用途 | 差异 |
|------|------|------|------|
| 完整图 | `build_trading_graph()` | 实盘/模拟 | 含辩论门控、辩论、风控、执行、日志 |
...
**辩论门控（debate_gate）**：
- 强共识（所有 Agent 方向一致 + 高置信度）→ 直接跳过辩论，进入 `enrich_verdict_context`
- 共同困惑（低置信度均值 + 低离散度）→ 跳过辩论（辩论无助于解决困惑）
- 低置信度均值 + 高离散度 = 真正的分歧 → 进入辩论轮次
- 渐进式过滤：辩论门控可将 LLM 调用从 13 次减少到 4-5 次

**辩论轮次**：
1. 每个 Agent 看到其他 3 个 Agent 的完整分析（各轮次内并行执行，`asyncio.gather`，延迟降低 4 倍）
2. 必须指出其他 Agent 最弱的论点
3. 必须捍卫自己的立场或说明被什么具体数据改变了
4. **反趋同规则**：不因"别人都这么想"而改变立场，必须有新证据
5. 固定 2 轮（避免人为趋同）
6. 辩论跳过 + 置信度平坦 + 无熔断 → Verdict 自动降级为加权投票（节省 1 次 LLM 调用）
...
**Redis 失败行为**：若 Redis 已配置但运行时不可用，系统采取保守拒绝策略（而非跳过）——Redis 依赖的检查（冷却、频率、日亏损）视为失败，拒绝交易。若 Redis 从未配置，则降级到内存状态管理，检查照常执行。
